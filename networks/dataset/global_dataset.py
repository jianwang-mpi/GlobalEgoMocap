...
import pickle
import os
from utils.utils import get_relative_global_pose, get_relative_global_pose_with_camera_matrix
import numpy as np
...
class AMASSDataset(Dataset):
    def __init__(self, data_path, frame_num, windows_size=1, is_train=True, fps=25, slide_window=True,
        ...
        self.relative_pose_seq_list = self.get_relative_global_pose_list(self.data_list, frame_num=self.frame_num,
                                                                         windows_size=self.windows_size,
                                                                         fps=fps)
    
    def __getitem__(self, i):
        data_i = self.relative_pose_seq_list[i]
        data_i = data_i.reshape((-1, 45))
        data_i = torch.from_numpy(data_i).float()
        return data_i
    
    def __len__(self):
        return len(self.relative_pose_seq_list)
    ...
    def get_local_poses(self, data_list):
        local_pose_list = []
        for data in data_list:
            local_pose_list.extend(data['local_pose_list'])
        return local_pose_list
    
    def get_relative_global_pose_list(self, data_list, frame_num=5, windows_size=5, fps=30):
        total_frame_number = frame_num * windows_size
        relative_global_pose_seq_list = []
        
        local_pose_seq_list = []
        camera_seq_list = []
        for seq_data in data_list:
        ...
        for local_pose_seq, camera_seq in tqdm(zip(local_pose_seq_list, camera_seq_list)):
            ...
            for i in range(0, len(relative_global_pose_seq), windows_size):
...
if __name__ == '__main__':
    # dataset = AMASSDataset(data_path='/home/wangjian/Develop/BodyPoseOptimization/AMASSDataConverter/pkls',
    #                        frame_num=5, windows_size=5,
    #                        is_train=False)
    # d = dataset[0]
    # print(d.shape)
